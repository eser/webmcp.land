...
const cli = meow(`
  Usage
    $ webmcp-land [command] [options]

  Commands
    (default)     Launch interactive TUI
    new <dir>     Create a new webmcp.land instance
  ...
  Examples
    $ npx webmcp.land
    $ npx webmcp.land new my-prompts
    $ webmcp-land
`, {
...
async function main() {
  ...
  if (command === 'new') {
    ...
    if (!directory) {
      ...
      console.error('   Usage: npx webmcp.land new <directory>\n');
      process.exit(1);
    }
  ...
  // Handle 'mcp' command - proxy to @eser/webmcp.land-mcp
  if (command === 'mcp') {
    const child = spawn('npx', ['-y', '@eser/webmcp.land-mcp', ...args], {
      stdio: 'inherit',
      shell: true,
    });
