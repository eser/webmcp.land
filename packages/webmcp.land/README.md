...
[![Playground](https://img.shields.io/badge/Playground-webmcp.land%2Ftyped--prompts--editor-blue)](https://webmcp.land/typed-prompts-editor)
...
`webmcp.land` solves these problems by providing a fluent, type-safe API that guides you through prompt construction with autocomplete, validation, and pre-built templates for common patterns.

## Installation

```bash
npm install webmcp.land
```
...
Scaffold a new webmcp.land deployment with a single command:

```bash
npx webmcp.land new my-prompt-library
```
...
```typescript
import { builder, chat, image, audio, variables, quality } from 'webmcp.land';
...
```typescript
import { variables } from 'webmcp.land';
...
```typescript
import { similarity } from 'webmcp.land';
...
```typescript
import { builder, fromPrompt, templates } from 'webmcp.land';
...
```typescript
import { templates } from 'webmcp.land';
...
```typescript
import { chatPresets } from 'webmcp.land';
...
import { variables, similarity, quality, parser } from 'webmcp.land';

// Direct builder imports
import { builder, chat, image, video, audio } from 'webmcp.land';
import { templates, chatPresets } from 'webmcp.land';

// Direct module imports (smallest bundle)
import { detect, normalize, compile } from 'webmcp.land/variables';
import { calculate, isDuplicate } from 'webmcp.land/similarity';
import { check, validate, getSuggestions } from 'webmcp.land/quality';
import { parse, toYaml, toJson } from 'webmcp.land/parser';
import { builder, templates } from 'webmcp.land/builder';
```
...
MIT © [Eser Ozvataf](https://github.com/eser)
