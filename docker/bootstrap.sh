...
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║                                                               ║"
echo "║   🚀 webmcp.land - MCP/WebMCP Service Registry              ║"
echo "║                                                               ║"
echo "╚═══════════════════════════════════════════════════════════════╝"
echo ""
...
EOF
    ...
    sleep 3
    su postgres -c "$PGBIN/createuser -s webmcp 2>/dev/null" || true
    su postgres -c "$PGBIN/createdb -O webmcp webmcp 2>/dev/null" || true
...
if [ ! -f "$BUILD_MARKER" ]; then
    ...
    # Build application
    echo "▶ Building webmcp.land (this may take a few minutes)..."
    pnpm run build
    echo "✓ Build complete"
...
# Start supervisord (manages PostgreSQL and the app)
...
if [ ! -f "$SEED_MARKER" ]; then
    ...
    if node --env-file=.env drizzle/seed.ts 2>/dev/null; then
        touch "$SEED_MARKER"
