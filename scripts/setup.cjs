...
/**
 * Interactive setup script for private webmcp.land clones.
...
# webmcp.land environment configuration
...
async function main() {
  ...
  p.intro(color.bgCyan(color.black(' webmcp.land - Private Clone Setup ')));
  ...
  const branding = await p.group({
    ...
    description: () => p.text({
      message: 'Description',
      placeholder: 'Collect, organize, and share MCP resources',
      defaultValue: 'Collect, organize, and share MCP resources',
    }),
    logo: () => p.text({
  ...
  if (!p.isCancel(setupDb) && setupDb) {
    ...
    s.start('Running database setup (drizzle migrate + seed)...');
    try {
