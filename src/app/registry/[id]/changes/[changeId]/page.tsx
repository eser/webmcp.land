...
import { changeRequests } from "@/lib/schema";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { DiffView } from "@/components/ui/diff-view";

import { ChangeRequestActions } from "@/components/resources/change-request-actions";
import { ReopenChangeRequestButton } from "@/components/resources/reopen-change-request-button";
import { DismissChangeRequestButton } from "@/components/resources/dismiss-change-request-button";
...
export default async function ChangeRequestPage({ params }: ChangeRequestPageProps) {
  ...
  const resourceId = extractResourceId(idParam);
  ...
  const isResourceOwner = session?.user?.id === changeRequest.resource.authorId;
  ...
  return (
    <div className="container max-w-3xl py-6">
      {/* Header */}
      <div className="mb-6">
        <Button render={<Link href={`/registry/${resourceId}`} />} variant="ghost" size="sm" className="mb-4 -ml-2">
      ...
      {/* Actions for resource owner */}
      {isResourceOwner && changeRequest.status === "PENDING" && (
        <div className="pt-4 border-t">
          <ChangeRequestActions changeRequestId={changeRequest.id} resourceId={resourceId} />
        </div>
      )}

      {/* Reopen button for rejected requests */}
      {isResourceOwner && changeRequest.status === "REJECTED" && (
        <div className="pt-4 border-t">
          <ReopenChangeRequestButton changeRequestId={changeRequest.id} resourceId={resourceId} />
        </div>
      )}
      ...
      {isChangeRequestAuthor && changeRequest.status === "PENDING" && (
        <div className="pt-4 border-t">
          <DismissChangeRequestButton changeRequestId={changeRequest.id} resourceId={resourceId} />
        </div>
      )}
    </div>
  );
