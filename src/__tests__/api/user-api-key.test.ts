...
describe("GET /api/user/api-key", () => {
  ...
  it("should return resourcesPublicByDefault setting", async () => {
    vi.mocked(getSession).mockResolvedValue({ user: { id: "user1" } } as never);
    vi.mocked(db.select).mockReturnValue(createChainMock([{
      apiKey: "wmcp_abc123",
      resourcesPublicByDefault: false,
    }]) as any);
    ...
    expect(response.status).toBe(200);
    expect(data.resourcesPublicByDefault).toBe(false);
  });
});
...
describe("PATCH /api/user/api-key", () => {
  ...
  it("should return 401 if not authenticated", async () => {
    ...
    const request = new Request("http://localhost:3000/api/user/api-key", {
      method: "PATCH",
      body: JSON.stringify({ resourcesPublicByDefault: true }),
    });
  ...
  it("should return 400 for missing resourcesPublicByDefault", async () => {
    vi.mocked(getSession).mockResolvedValue({ user: { id: "user1" } } as never);
  ...
  it("should return 400 for number value", async () => {
    ...
    const request = new Request("http://localhost:3000/api/user/api-key", {
      method: "PATCH",
      body: JSON.stringify({ resourcesPublicByDefault: 1 }),
    });
    const response = await PATCH(request as unknown as NextRequest);
    const data = await response.json();
  ...
  it("should update resourcesPublicByDefault to true", async () => {
    vi.mocked(getSession).mockResolvedValue({ user: { id: "user1" } } as never);
    vi.mocked(db.update).mockReturnValue(createChainMock([]) as any);
  ...
  it("should update resourcesPublicByDefault to false", async () => {
    vi.mocked(getSession).mockResolvedValue({ user: { id: "user1" } } as never);
    vi.mocked(db.update).mockReturnValue(createChainMock([]) as any);

    const request = new Request("http://localhost:3000/api/user/api-key", {
      method: "PATCH",
      body: JSON.stringify({ resourcesPublicByDefault: false }),
    });
    const response = await PATCH(request as unknown as NextRequest);
    const data = await response.json();
