  ...
  return (
    <div className="flex items-center gap-1 p-1 border-b bg-muted/30">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger render={<Button variant="ghost" size="sm" className="h-7 text-xs gap-1.5" />}>
            <Variable className="h-3.5 w-3.5" />
            {t("resources.insertVariable")}
        </PopoverTrigger>
        <PopoverContent className="w-72" align="start">
          <div className="space-y-3">
            <div className="space-y-1">
              <Label htmlFor="var-name" className="text-xs">{t("resources.variableName")}</Label>
              <Input
                id="var-name"
                value={name}
            ...
            <div className="space-y-1">
              <Label htmlFor="var-default" className="text-xs">{t("resources.variableDefault")}</Label>
              <Input
                id="var-default"
                value={defaultValue}
                onChange={(e) => setDefaultValue(e.target.value)}
                onKeyDown={handleKeyDown}
                placeholder={t("resources.variableDefaultPlaceholder")}
                className="h-8 text-sm"
              />
            </div>
