        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            <p className="text-sm font-medium text-amber-700 dark:text-amber-400">
              {t("resources.variableWarningTitle")}
            </p>
            <p className="text-xs text-muted-foreground">
              {t("resources.variableWarningDescription")}
            </p>
          </div>
          <Button
            type="button"
        ...
        <div className="flex flex-wrap gap-1.5 mt-1">
          ...
          {detected.length > 5 && (
            <Badge variant="secondary" className="text-xs">
              +{detected.length - 5} {t("resources.more")}
            </Badge>
          )}
        </div>
        <p className="text-xs text-muted-foreground mt-1">
          {t("resources.supportedFormat")}: <code className="bg-muted px-1 py-0.5 rounded">{`\${variable}`}</code> {t("resources.or")} <code className="bg-muted px-1 py-0.5 rounded">{`\${variable:default}`}</code>
        </p>
      </AlertDescription>
    </Alert>
  );
