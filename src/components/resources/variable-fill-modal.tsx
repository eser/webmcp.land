...
export function VariableFillModal({
  ...
  const handleCopy = async () => {
    try {
      ...
      analyticsResource.fillVariables(resourceId);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-base">{t("common.variables")}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex justify-end gap-2 pt-2">
          {mode === "copy" ? (
            ...
            <RunResourceButton content={finalContent} size="sm" categoryName={categoryName} parentCategoryName={parentCategoryName} />
          )}
        </div>
      </DialogContent>
    </Dialog>
