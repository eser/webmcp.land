...
function FlowGraph({ nodes, edges, currentResourceId, currentUserId, isAdmin, onNodeClick, onNodeDelete, translations }: FlowGraphProps) {
  ...
  }, [nodes, edges, currentResourceId, onNodeClick, handleNodeEnter, handleNodeLeave, translations]);
...
export function ResourceConnections({
  ...
  const handleRemoveFromFlow = useCallback(async (node: FlowGraphNode) => {
    ...
    try {
      ...
      for (const edge of edgesToDelete) {
        ...
        const res = await fetch(`/api/resources/${edge.source}/connections`, { cache: "no-store" });
        if (res.ok) {
          const data = await res.json();
          const conn = data.outgoing?.find((c: OutgoingConnection) => c.targetId === edge.target);
          if (conn) {
            await fetch(`/api/resources/${edge.source}/connections/${conn.id}`, {
              method: "DELETE",
              cache: "no-store",
            });
          }
  ...
  const handleDeleteConnection = useCallback(async (connectionId: string) => {
    try {
      const res = await fetch(
        `/api/resources/${resourceId}/connections/${connectionId}`,
      ...
      if (res.ok) {
        ...
        toast.success(t("connectedResources.connectionDeleted"));
        // Also refetch to ensure sync with server
        fetchConnections();
      } else {
        const data = await res.json();
        toast.error(data.error || t("connectedResources.deleteFailed"));
      }
    } catch {
      toast.error(t("connectedResources.deleteFailed"));
    }
  }, [resourceId, t, fetchConnections]);

  const handleConnectionAdded = () => {
    toast.success(t("connectedResources.connectionAdded"));
    fetchConnections();
  };

  const getResourceLink = (resource: { slug: string | null; id: string }) => {
    return getResourceUrl(resource.id, resource.slug);
  };
      ...
      outgoingNodes
        ...
        .on("click", (event, d) => {
          ...
          const link = links.find(
            (l) => l.target.id === d.id && l.source.id === resourceId
          );
          if (link) {
  ...
  }, [isLoading, incoming, outgoing, resourceId, resourceTitle, canEdit, router, handleDeleteConnection]);
  ...
  if (buttonOnly) {
    ...
    return (
      ...
      >
        ...
        {t("connectedResources.addResourceFlow")}
  ...
  if (sectionOnly) {
    ...
    return (
      <div className="w-full mt-4 space-y-3">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold">{t("connectedResources.title")}</h3>
            </div>
            ...
            {canEdit && (
              <>
                ...
                >
                  ...
                  <span className="hidden sm:inline">{t("connectedResources.addPrevious")}</span>
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                ...
                >
                  ...
                  <span className="hidden sm:inline">{t("connectedResources.addNext")}</span>
                </Button>
              </>
            )}
            {hasConnections && workflowLink && (() => {
              ...
              return (
                ...
                >
                  ...
                  <span className="hidden sm:inline">{t("connectedResources.testWorkflow")}</span>
                  {domain && (
        ...
        {!hasConnections ? (
          <p className="text-sm text-muted-foreground">{t("connectedResources.noConnections")}</p>
        ) : hasFullFlow ? (
          /* Full Flow Graph Display with D3 */
          <div className="w-full space-y-2">
            <p className="text-xs text-muted-foreground text-center mb-3">{t("connectedResources.fullFlow")}</p>
            <FlowGraph
              nodes={flowNodes}
              edges={flowEdges}
              ...
              isAdmin={isAdmin}
              onNodeClick={(node) => router.push(getResourceLink(node))}
              onNodeDelete={canEdit ? handleRemoveFromFlow : undefined}
              translations={{
                outputText: t("connectedResources.outputText"),
                outputImage: t("connectedResources.outputImage"),
                outputVideo: t("connectedResources.outputVideo"),
                outputAudio: t("connectedResources.outputAudio"),
                outputStructured: t("connectedResources.outputStructured"),
                outputSkill: t("connectedResources.outputSkill"),
          ...
          <div className="w-full">
            {incoming.length > 0 && (
              <p className="text-xs text-muted-foreground mb-2 text-center">{t("connectedResources.previousSteps")}</p>
            )}
            <div className="w-full">
              <svg ref={svgRef} className="w-full" />
            </div>
            {outgoing.length > 0 && (
              <p className="text-xs text-muted-foreground mt-2 text-center">{t("connectedResources.nextSteps")}</p>
            )}
          </div>
        )}
        </div>
        ...
        <div className="w-full mt-4 space-y-4 rounded-lg border bg-card p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">{t("connectedResources.description")}</p>
            </div>
            {canEdit && (
              <div className="flex gap-2">
                <Button
